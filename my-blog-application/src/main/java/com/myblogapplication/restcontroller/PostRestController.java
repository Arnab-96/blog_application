...
public class PostRestController {
    ...
    @GetMapping("/filters")
    public ResponseEntity<?> postFilter(@RequestParam(value = "author",required = false) String author,
                                        @RequestParam(value = "tagId",required = false) List<Integer> tagIds,
                                        @RequestParam(value = "Date",required = false) String date){
        List<Post> filteredPosts = postService.findFilteredPosts(author,tagIds,date);
    ...
    public ResponseEntity<List<Post>> getPostsByPagination(@RequestParam(value = "start") int page,
        ...
        Page<Post> postPage = postService.getAllPosts(page/9+1,limit);
